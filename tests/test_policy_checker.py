...
from policytools.policy_checker import PolicyChecker
from policytools.check_response import CheckResponse
from tests.aws_client_stub import AwsClientStub
...
def test_policy_check_actions_expect_no_denied_actions():
    checker = PolicyChecker(
        AwsClientStub(get_json_fixture_as_string('s3-resource/resp.s3.GetObject.allowed.json'))
    )
    deny_putobject_policy = get_json_fixture_as_string('s3-resource/policy.s3.PutObject.deny.json')
    allow_s3_policy = get_json_fixture_as_string('s3-resource/policy.s3-star.allow.json')
    checker.append_policy('policy.s3.PutObject.deny.json', deny_putobject_policy)
    checker.append_policy('policy.s3-star.allow.json', allow_s3_policy)
...
def test_policy_check_actions_expect_no_allowed_actions():
    checker = PolicyChecker(
        AwsClientStub(get_json_fixture_as_string('s3-resource/resp.s3.PutObject.explicitDeny.json'))
    )
    allow_s3_policy = get_json_fixture_as_string('s3-resource/policy.s3-star.allow.json')
    deny_s3_putobject_policy = get_json_fixture_as_string('s3-resource/policy.s3.PutObject.deny.json')
    checker.append_policy('policy.s3-star.allow.json', allow_s3_policy)
    checker.append_policy('policy.s3.PutObject.deny.json', deny_s3_putobject_policy)
...
def test_policy_check_actions_expect_implicit_denied_action():
    checker = PolicyChecker(
        AwsClientStub(get_json_fixture_as_string('s3-resource/resp.ec2.CreateInstance.implicitDeny.json'))
    )
    deny_s3_putobject_policy = get_json_fixture_as_string('s3-resource/policy.s3.PutObject.deny.json')
    checker.append_policy('policy.s3.PutObject.deny.json', deny_s3_putobject_policy)
